...
export const displayLabeledChildWithLabel = <S, C, K extends keyof C> ( st: RunbookState<S, C>, props: RunbookProps<any>, r: RunbookComponent<S, C[K]>, label: ( def: string ) => string, k: K ): JSX.Element => {
  let withLabel = labelAnd ( label ( decamelize ( k.toString () ) ), r );
  return displayChild ( st, props, k, withLabel );
};
export const displayLabeledChild = <S, C, K extends keyof C> ( st: RunbookState<S, C>, props: RunbookProps<any>,
                                                               r: RunbookComponent<S, C[K]>,
                                                               k: K ): JSX.Element =>
  displayLabeledChildWithLabel ( st, props, r, l => l, k );

export const displayLabeledQueryChild = <S, C, K extends keyof C> ( st: RunbookState<S, C>, props: RunbookProps<any>,
