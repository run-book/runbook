...
export type Mereology = {
  direct?: NameAnd<any>,
  bound?: NameAnd<NameAnd<Mereology>>
}
export type BoundMereology = {
  direct: NameAnd<any>,
  fromBindings: NameAnd<NameAnd<BoundMereology>>
}
export function toMereology ( defn: MereologyDefn ): Mereology {
...
export const fromMereology = ( mereology: Mereology ): FromMereologyFn => ( existing: NameSpaceAndValue[], searchNameSpace: string, searchValue: string ) => {
  ...
  const fromBindings: NameAnd<any>[] = existing.map ( ( { namespace, value } ) => {
    ...
    const childMereology: Mereology = mereology.bound[ namespace ]?.[ value ]
